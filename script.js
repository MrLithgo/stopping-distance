document.addEventListener('DOMContentLoaded', function() {
    ...
    reactionTestArea.onclick = function() {
        if (reactionTestState === 'idle') {
            ...
            if (currentTestNumber < 3) {
                ...
                reactionTestText.textContent = `Test ${currentTestNumber}/3: ${reactionTime.toFixed(3)} seconds. Get ready for next test...`;
                reactionTestState = 'idle';
                ...
                measuredReactionTime.textContent = averageReactionTime.toFixed(3) + ' seconds';

                reactionTestArea.style.backgroundColor = '#DBEAFE'; // Blue background
                reactionTestText.textContent = `Average reaction time: ${averageReactionTime.toFixed(3)} seconds. Click to try again.`;
                reactionTestState = 'idle';
    ...
    useMyReactionBtn.onclick = function() {
        if (measuredReactionTimeValue !== null) {
            reactionSlider.value = Math.min(Math.max(measuredReactionTimeValue, 0.2), 2.0);
            reactionValue.textContent = measuredReactionTimeValue.toFixed(2) + ' seconds';
            updateDistances();
        }
    };
    ...
    function updateDistances() {
        ...
        // Convert km/h to m/s for calculations (1 km/h = 0.27778 m/s)
        const speedMS = speed * 0.27778;

        // Calculate thinking distance (speed × reaction time)
        const thinkingDist = speedMS * reactionTimeSeconds;

        // Calculate braking distance with road condition factor
        // Using formula: v² / (2 × μ × g) where μ is friction coefficient and g is gravity (9.81 m/s²)
        ...
        // Update display values (rounded to 1 decimal place)
        ...
        safeSetTimeout(() => {
            ...
            safeSetTimeout(() => {
                ...
                safeSetTimeout(() => {
                    ...
                    const brakingTime = Math.max(1.5, speedMS / (frictionCoefficient * 9.81));
